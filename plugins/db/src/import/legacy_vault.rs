...
use meeki_fs_sync_core::frontmatter::ParsedDocument;
...
pub async fn import_legacy_vault(
    ...
    meeki_db_app::begin_legacy_import_run(pool, &run_id, &source_root, dry_run).await?;

    let discovery = match discover_sources(vault_base) {
        Ok(discovery) => discovery,
        Err(error) => {
            meeki_db_app::fail_legacy_import_run(pool, &run_id, &error.to_string()).await?;
    ...
    for source in discovery.files {
        let bytes = match std::fs::read(&source.path) {
            ...
            Err(error) => {
                ...
                meeki_db_app::record_legacy_import_error(
                    pool,
                    LegacyImportItem {
                        id: &item_id,
                        run_id: &run_id,
        ...
        if !dry_run
            ...
            && meeki_db_app::legacy_source_already_imported(
                pool,
            ...
            .await?
        {
            meeki_db_app::record_legacy_import_unchanged(
                pool,
                LegacyImportItem {
                    id: &item_id,
                    run_id: &run_id,
        ...
        match parse_source(vault_base, &source, &bytes, &source_sha256) {
            Ok(mut batch) => {
                ...
                meeki_db_app::apply_legacy_import_item(pool, item, &batch, dry_run).await?;
            }
            Err(error) => {
                meeki_db_app::record_legacy_import_error(pool, item, &error).await?;
            }
        }
    }

    meeki_db_app::finish_legacy_import_run(pool, &run_id).await?;
    Ok(run_id)
}
...
mod tests {
    ...
    async fn divergent_nonempty_summaries_recover_after_a_partial_import() {
        ...
        meeki_db_app::begin_legacy_import_run(
            db.pool(),
            failed_run_id,
        ...
        for (item_id, source_path, source_kind, source_hash, batch) in [
            ...
            meeki_db_app::apply_legacy_import_item(
                db.pool(),
                LegacyImportItem {
                    id: item_id,
        ...
        assert_eq!(
            meeki_db_app::finish_legacy_import_run(db.pool(), failed_run_id)
                .await
                .unwrap(),
