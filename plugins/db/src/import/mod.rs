...
fn resolve_startup_vault_base<R: tauri::Runtime>(
    ...
    let settings_base = meeki_storage::global::compute_default_base(bundle_id)
...
mod tests {
    ...
    async fn finish_issue_run(
        ...
        meeki_db_app::begin_legacy_import_run(pool, run_id, "/vault", false)
            .await
            .unwrap();
        sqlx::query(
        ...
        meeki_db_app::finish_legacy_import_run(pool, run_id)
            .await
            .unwrap()
    }
    ...
    async fn failed_source_scan_is_left_for_explicit_retry() {
        ...
        meeki_db_app::begin_legacy_import_run(db.pool(), "failed-run", "/vault", false)
            .await
            .unwrap();
        meeki_db_app::fail_legacy_import_run(db.pool(), "failed-run", "permission denied")
            .await
            .unwrap();
    ...
    async fn verified_file_import_survives_cloudsync_reopen_without_rerun() {
        ...
        assert_eq!(state_before.2, meeki_db_app::LEGACY_IMPORTER_VERSION);
        assert!(legacy_migration_verified(db.pool()).await.unwrap());
        assert!(!legacy_import_attempt_required(db.pool()).await.unwrap());
