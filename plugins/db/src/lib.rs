...
impl From<CloudsyncWorkspaceProjection> for meeki_db_app::CloudsyncWorkspaceProjection {
...
pub fn init<R: tauri::Runtime>(
    db: std::sync::Arc<meeki_db_core::Db>,
...
pub fn init_with_cloudsync<R: tauri::Runtime>(
    db: std::sync::Arc<meeki_db_core::Db>,
    startup_config: Option<meeki_db_core::CloudsyncRuntimeConfig>,
) -> tauri::plugin::TauriPlugin<R> {
    ...
    tauri::plugin::Builder::new(PLUGIN_NAME)
        ...
        .setup(move |app, _| {
            meeki_tauri_utils::block_on(meeki_db_app::prepare_schema(db.as_ref()))?;
            meeki_tauri_utils::block_on(import::import_legacy_data(app.app_handle(), db.pool()))?;
            if let Some(config) = startup_config.clone() {
                let migration_verified =
                    meeki_tauri_utils::block_on(import::legacy_migration_verified(db.pool()))?;
                if !migration_verified {
                    tracing::warn!(
                        "startup CloudSync configuration skipped until legacy migration is verified"
                    );
                } else if let Err(error) =
                    meeki_tauri_utils::block_on(db.cloudsync_configure(config))
                {
...
mod test {
    ...
    use meeki_db_reactive::QueryEventSink;
    use serde_json::json;
    ...
    async fn setup_runtime_with_cloudsync(
        ...
        let db = meeki_db_core::Db::open(meeki_db_core::DbOpenOptions {
            storage: meeki_db_core::DbStorage::Local(&db_path),
            cloudsync_enabled,
    ...
    async fn execute_proxy_applies_app_schema_before_run() {
        ...
        runtime
            .execute_proxy(
                ...
                meeki_db_execute::ProxyQueryMethod::Run,
            )
            .await
            .unwrap();
    ...
    async fn open_memory_app_db_subscribe_sees_app_schema() {
        ...
        assert!(matches!(
            ...
            meeki_db_reactive::DependencyAnalysis::Reactive { .. }
        ));
        ...
        runtime
            .configure_cloudsync(
                serde_json::json!({
                    ...
                    "tables": meeki_db_app::cloudsync_table_registry(),
    ...
    async fn account_binding_is_durable_without_rekeying_local_rows() {
        let (_dir, runtime) = setup_runtime().await;
        let local_workspace = meeki_db_app::ensure_cloudsync_workspace_binding(runtime.pool())
            .await
            .unwrap();
        sqlx::query(
    ...
    async fn token_configuration_rejects_local_only_runtime_before_rekeying() {
        ...
        assert!(matches!(
            error,
            crate::Error::Cloudsync(meeki_db_core::CloudsyncRuntimeError::Unavailable)
        ));
    ...
    async fn invalid_projection_does_not_claim_an_unbound_database() {
        ...
        let error = runtime
            .configure_cloudsync_token_with_projection(
                ...
                "token".to_string(),
                "user-a".to_string(),
                Some(meeki_db_app::CloudsyncWorkspaceProjection {
        ...
        assert!(matches!(
            ...
            crate::Error::CloudsyncWorkspace(
                meeki_db_app::CloudsyncWorkspaceError::InvalidWorkspaceProjection
            )
        ));
    ...
    async fn token_refresh_restarts_pending_full_resync_with_new_credentials() {
        ...
        let projection = meeki_db_app::CloudsyncWorkspaceProjection {
            account_user_id: "user-a".to_string(),
            personal_workspace_id: "user-a".to_string(),
            workspaces: vec![meeki_db_app::CloudsyncWorkspaceProjectionEntry {
                id: "user-a".to_string(),
        ...
        assert_eq!(
            auth,
            meeki_db_core::CloudsyncAuth::Token {
    ...
    async fn token_configuration_projects_server_workspaces_after_account_claim() {
        ...
        let projection = meeki_db_app::CloudsyncWorkspaceProjection {
            ...
            personal_workspace_id: "user-a".to_string(),
            workspaces: vec![
                meeki_db_app::CloudsyncWorkspaceProjectionEntry {
                    id: "user-a".to_string(),
                    owner_user_id: "user-a".to_string(),
                    kind: "personal".to_string(),
                    name: "Personal".to_string(),
        ...
        assert!(
            meeki_db_app::cloudsync_write_filter_installed(runtime.pool(), "user-a")
                .await
                .unwrap()
        );
    ...
    async fn token_configuration_account_mismatch_preserves_workspace_projection() {
        ...
        meeki_db_app::replace_cloudsync_workspace_projection(
            runtime.pool(),
            &meeki_db_app::CloudsyncWorkspaceProjection {
                account_user_id: "user-a".to_string(),
                personal_workspace_id: "user-a".to_string(),
                workspaces: vec![meeki_db_app::CloudsyncWorkspaceProjectionEntry {
                    id: "user-a".to_string(),
