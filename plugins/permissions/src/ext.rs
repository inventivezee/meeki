...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Permissions<'a, R, M> {
    fn audio_provider(&self) -> Option<Arc<dyn meeki_audio::AudioProvider>> {
        self.manager
            .try_state::<Arc<dyn meeki_audio::AudioProvider>>()
            .map(|s| Arc::clone(&*s))
    }

    fn require_audio(&self) -> Result<Arc<dyn meeki_audio::AudioProvider>, crate::Error> {
    ...
    async fn check_system_audio(&self) -> Result<PermissionStatus, crate::Error> {
        #[cfg(target_os = "macos")]
        return check!("system_audio", meeki_tcc::audio_capture_permission_status());
    ...
    async fn check_screen_recording(&self) -> Result<PermissionStatus, crate::Error> {
        ...
        return check!(
            "screen_recording",
            meeki_tcc::screen_capture_permission_status()
        );
    ...
    async fn request_screen_recording(&self) -> Result<(), crate::Error> {
        #[cfg(target_os = "macos")]
        {
            let _ = meeki_tcc::request_screen_capture_permission();
        }
    ...
    async fn reset_tcc(&self, service: &str) {
        ...
        let bundle_id = if cfg!(debug_assertions) {
            match meeki_bundle::get_ancestor_bundle_id() {
                Some(id) => {
