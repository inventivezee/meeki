...
pub fn available_providers() -> Vec<CalendarProviderType> {
    meeki_calendar::available_providers()
}
...
pub async fn is_provider_enabled<R: tauri::Runtime>(
    ...
    meeki_calendar::is_provider_enabled(&config.api_base_url, token.as_deref(), apple, provider)
        .await
        .map_err(Into::into)
}
...
pub async fn list_connection_ids<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<Vec<meeki_calendar::ProviderConnectionIds>, Error> {
    let config = app.state::<crate::PluginConfig>();
    let token = access_token(&app)?;
    let apple = is_apple_authorized(&app).await?;
    meeki_calendar::list_connection_ids(&config.api_base_url, token.as_deref(), apple)
        .await
        .map_err(Into::into)
}
...
pub async fn list_calendars<R: tauri::Runtime>(
    ...
    meeki_calendar::list_calendars(&config.api_base_url, &token, provider, &connection_id)
        .await
        .map_err(Into::into)
}
...
pub fn open_calendar<R: tauri::Runtime>(
    ...
    meeki_calendar::open_calendar(provider).map_err(Into::into)
}
...
pub fn create_event<R: tauri::Runtime>(
    ...
    meeki_calendar::create_event(provider, input).map_err(Into::into)
}

#[tauri::command]
#[specta::specta]
pub fn parse_meeting_link(text: String) -> Option<String> {
    meeki_calendar::parse_meeting_link(&text)
}
...
fn require_access_token<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Result<String, Error> {
    ...
    match token {
        ...
        _ => Err(meeki_calendar::Error::NotAuthenticated.into()),
    }
}
...
async fn is_apple_authorized<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Result<bool, Error> {
    #[cfg(target_os = "macos")]
    {
        let status = app
            ...
            .await
            .map_err(|e| meeki_calendar::Error::Api(e.to_string()))?;
        Ok(matches!(
            status,
