...
fn zoom() -> meeki_detect::InstalledApp {
...
fn aqua_voice() -> meeki_detect::InstalledApp {
    meeki_detect::InstalledApp {
...
fn screen_studio() -> meeki_detect::InstalledApp {
    meeki_detect::InstalledApp {
...
fn snagit_2024() -> meeki_detect::InstalledApp {
    meeki_detect::InstalledApp {
...
fn slack() -> meeki_detect::InstalledApp {
    meeki_detect::InstalledApp {
...
impl Harness {
    ...
    fn mic_started(&self, app: meeki_detect::InstalledApp) {
        handle_detect_event(
            &self.env,
            &self.state,
            meeki_detect::DetectEvent::MicStarted(vec![app]),
        );
    }

    fn mic_stopped(&self, app: meeki_detect::InstalledApp) {
        handle_detect_event(
            &self.env,
            &self.state,
            meeki_detect::DetectEvent::MicStopped(vec![app]),
        );
    }
...
async fn test_multiple_apps_start_simultaneously() {
    ...
    handle_detect_event(
        &h.env,
        &h.state,
        meeki_detect::DetectEvent::MicStarted(vec![zoom(), slack()]),
    );
...
async fn test_stop_all_apps_simultaneously() {
    ...
    handle_detect_event(
        &h.env,
        &h.state,
        meeki_detect::DetectEvent::MicStopped(vec![zoom(), slack()]),
    );
    h.take_events();
