...
fn intersect_mic_active_bundle_ids(
    ...
    current_mic_apps: &[meeki_detect::InstalledApp],
) -> Vec<String> {
...
) -> Result<Vec<meeki_detect::InstalledApp>, String> {
...
pub(crate) async fn list_mic_using_applications<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<Vec<meeki_detect::InstalledApp>, crate::Error> {
...
) -> Result<Vec<meeki_detect::MeetingAccessibilityInspection>, String> {
    Ok(meeki_detect::inspect_meeting_accessibility())
}
...
pub(crate) async fn send_meeting_chat_message<R: tauri::Runtime>(
...
) -> Result<meeki_detect::MeetingChatSendResult, String> {
    ...
    Ok(meeki_detect::send_meeting_chat_message(
        message,
        verified_bundle_ids,
    ))
}
...
pub(crate) async fn capture_meeting_chat_messages<R: tauri::Runtime>(
...
) -> Result<meeki_detect::MeetingChatCaptureResult, String> {
    let current_mic_apps = app
        .detect()
        .list_mic_using_applications()
        .map_err(|error| error.to_string())?;
    ...
    Ok(meeki_detect::capture_meeting_chat_messages(
        verified_bundle_ids,
    ))
}
...
pub(crate) async fn get_preferred_languages<R: tauri::Runtime>(
    ...
    Ok(meeki_detect::get_preferred_languages()
        .into_iter()
        .map(|l| l.bcp47_code())
        .collect())
}
...
pub(crate) async fn get_current_locale_identifier<R: tauri::Runtime>(
    ...
    Ok(meeki_detect::get_current_locale_identifier())
}
