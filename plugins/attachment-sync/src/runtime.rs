...
const MAX_PLAINTEXT_BYTES: u64 = meeki_e2ee::ATTACHMENT_BLOB_MAX_PLAINTEXT_BYTES;
...
fn delete_source_changed(error: &Error) -> bool {
    match error {
        ...
        | Error::E2ee(meeki_e2ee::AttachmentBlobError::SourceMismatch) => true,
...
fn attachment_paths<R: Runtime>(
    ...
    let vault_base = app
        ...
        .into_std_path_buf();
    let session_candidate = meeki_fs_sync_core::FsSyncCore::new(vault_base.clone())
        .resolve_session_dir(&attachment.session_id)
        .map_err(|_| Error::LocalAttachmentUnavailable)?;
    let session_dir = meeki_fs_sync_core::resolve_path_inside_base(&vault_base, &session_candidate)
        .map_err(|_| Error::LocalAttachmentUnavailable)?;
    let path = meeki_fs_sync_core::resolve_path_inside_base(
        &session_dir,
...
fn seal_delete_guard(
    ...
    {
        Ok(metadata) => metadata,
        Err(meeki_e2ee::AttachmentBlobError::Io(error))
...
fn unique_attachment_conflict_path(destination: &Path) -> Result<PathBuf> {
    ...
    let filename = destination
        ...
        .ok_or(Error::LocalAttachmentUnavailable)?
        .to_string_lossy();
    Ok(parent.join(format!("{filename}.meeki-conflict-{}", Uuid::new_v4())))
}
...
mod tests {
    ...
    fn delete_backup_refs_are_stable_for_persisted_job_metadata() {
        let key = meeki_e2ee::RecoveryKey::generate()
            .unwrap()
