...
use meeki_transcript::{RenderTranscriptRequest, RenderedTranscriptSegment};
use meeki_transcription_core::listener2 as listener2_core;
...
pub async fn is_supported_languages_live<R: tauri::Runtime>(
    ...
    let languages_parsed = languages
        .iter()
        .map(|s| meeki_language::Language::from_str(s))
...
pub async fn render_transcript_segments(
    ...
    Ok(meeki_transcript::render_transcript_segments(params))
}
