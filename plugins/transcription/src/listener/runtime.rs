use meeki_transcription_core::listener::ListenerRuntime;
...
use meeki_transcription_core::listener::State as RootState;
use meeki_transcription_core::listener::actors::{RootActor, RootMsg};
...
impl meeki_storage::StorageRuntime for TauriRuntime {
    fn global_base(&self) -> Result<std::path::PathBuf, meeki_storage::Error> {
        self.app
            .settings()
            .global_base()
            .map(|p| p.into_std_path_buf())
            .map_err(|_| meeki_storage::Error::DataDirUnavailable)
    }
...
impl ListenerRuntime for TauriRuntime {
    fn emit_lifecycle(&self, event: meeki_transcription_core::listener::SessionLifecycleEvent) {
        use tauri_plugin_tray::TrayPluginExt;
        match &event {
            meeki_transcription_core::listener::SessionLifecycleEvent::Active { error, .. } => {
        ...
        let capture_event = match event {
            meeki_transcription_core::listener::SessionLifecycleEvent::Active {
                session_id,
                ...
                let requested_live_transcription = requested_transcription_mode
                    == meeki_transcription_core::listener::TranscriptionMode::Live;
    ...
    fn emit_progress(&self, event: meeki_transcription_core::listener::SessionProgressEvent) {
        if let Err(error) = CaptureStatusEvent::from(event).emit(&self.app) {
            tracing::error!(?error, "failed_to_emit_progress_event");
        }
    }

    fn emit_error(&self, event: meeki_transcription_core::listener::SessionErrorEvent) {
        if let Err(error) = CaptureStatusEvent::from(event).emit(&self.app) {
            tracing::error!(?error, "failed_to_emit_error_event");
        }
    }

    fn emit_data(&self, event: meeki_transcription_core::listener::SessionDataEvent) {
