...
pub fn authorization_status() -> Result<String, Error> {
    #[cfg(target_os = "macos")]
    {
        let status = meeki_apple_todo::Handle::authorization_status();
        Ok(format!("{:?}", status))
    }
...
pub fn request_full_access() -> Result<bool, Error> {
    #[cfg(target_os = "macos")]
    {
        Ok(meeki_apple_todo::Handle::request_full_access())
    }
...
pub fn list_todo_lists() -> Result<Vec<ReminderList>, Error> {
    #[cfg(target_os = "macos")]
    {
        let handle = meeki_apple_todo::Handle;
        ...
        ReadPath::Apple(path) => {
            #[cfg(target_os = "macos")]
            {
                ...
                match handle.read_path(path)? {
                    meeki_apple_todo::ReadPathResult::Lists(items) => {
                        Ok(ReadPathResult::ReminderLists(items))
                    }
                    meeki_apple_todo::ReadPathResult::Reminders(items) => {
...
pub async fn github_issue_detail(
    ...
    repo: String,
    number: u64,
) -> Result<meeki_github_issues::Issue, Error> {
...
pub async fn github_issue_comments(
    ...
    repo: String,
    number: u64,
) -> Result<Vec<meeki_github_issues::IssueComment>, Error> {
