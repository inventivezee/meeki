use meeki_github_issues::{GitHubIssuesClient, Issue, IssueComment};
...
impl PublicGitHubHttpClient {
    fn new() -> Result<Self, Error> {
        ...
        headers.insert(
            ...
            reqwest::header::HeaderValue::from_static("meeki-desktop"),
        );
...
fn unsupported_request(method: &str) -> Result<Vec<u8>, meeki_http::Error> {
...
impl meeki_http::HttpClient for PublicGitHubHttpClient {
    async fn get(&self, path: &str) -> Result<Vec<u8>, meeki_http::Error> {
    ...
    async fn post(
        ...
        _body: Vec<u8>,
        _content_type: &str,
    ) -> Result<Vec<u8>, meeki_http::Error> {
        unsupported_request("POST")
    }

    async fn put(&self, _path: &str, _body: Vec<u8>) -> Result<Vec<u8>, meeki_http::Error> {
        unsupported_request("PUT")
    }

    async fn patch(&self, _path: &str, _body: Vec<u8>) -> Result<Vec<u8>, meeki_http::Error> {
        unsupported_request("PATCH")
    }

    async fn delete(&self, _path: &str) -> Result<Vec<u8>, meeki_http::Error> {
