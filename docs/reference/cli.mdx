---
title: "CLI reference"
description: "Commands and global options available in the Meeki CLI."
---
...
| --- | --- | --- |
| `--base DIR` | `MEEKI_BASE` | Use `DIR/app.db`. |
| `--db-path FILE` | `MEEKI_DB_PATH` | Use an explicit SQLite file. |
| `--json` | — | Emit a versioned JSON response or machine-readable JSON error. |

`--db-path` takes precedence over `--base`. Without either option, the CLI uses Meeki's platform application-data directory and recognized legacy locations.
