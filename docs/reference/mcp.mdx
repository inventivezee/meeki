---
title: "MCP reference"
description: "Tools, resources, limits, and transport details exposed by Meeki MCP."
---

Run the server with `meeki mcp`. It uses the MCP `2024-11-05` protocol over stdio. Every tool is read-only, non-destructive, idempotent, and local-only.
...
| Parameter | Type | Default | Notes |
| --- | --- | --- | --- |
| `meeting_id` | string | required | A returned Meeki meeting ID. |
...
| --- | --- | --- |
| `meeki://meetings/{meeting_id}` | `text/markdown` | Meeting detail without transcript text. |
| `meeki://meetings/{meeting_id}/transcript{?offset,limit}` | `text/plain` | A bounded transcript page. |
| `meeki://series/{series_id}` | `text/markdown` | Up to 100 meetings in a recurring series. |
