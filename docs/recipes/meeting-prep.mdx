...
description: "Build a focused meeting brief from recent Meeki context."
---
...
<Steps>
  <Step title="Find the latest meeting">
    ...
    ```bash
    meeki --json meetings list --query "project sync" --limit 10
    ```
  ...
  <Step title="Review recurring history">
    ...
    ```bash
    meeki --json meetings history MEETING_ID --limit 5 --offset 0
    ```
