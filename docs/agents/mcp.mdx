...
description: "Connect an MCP client to local Meeki meeting context over stdio."
---

The MCP server runs inside the `meeki` executable and communicates over standard input and output. It does not open a port or send meeting data to a hosted service.
...
If `meeki` is not on the client's `PATH`, use the executable's absolute path as `command`. Restart the client after changing its configuration.
