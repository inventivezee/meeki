...
use meeki_agent_access::{DEFAULT_TRANSCRIPT_LIMIT, MAX_TRANSCRIPT_LIMIT};

#[derive(Debug, Parser)]
#[command(name = "meeki", version, about = "Query local Meeki meeting data")]
pub struct Args {
    #[arg(
        long,
        global = true,
...
pub enum Command {
    ...
    /// Run the read-only Meeki MCP server over stdio
    Mcp,
}
...
mod tests {
    ...
    fn parses_meeting_list_filters() {
        let args = Args::parse_from([
            "meeki", "--json", "meetings", "list", "--query", "planning", "--limit", "10",
        ]);
    ...
    fn public_docs_and_skill_cover_the_command_contract() {
        ...
        let skill = concat!(
            include_str!("../../../skills/meeki/references/cli.md"),
            include_str!("../../../skills/meeki/references/setup.md"),
        );
        ...
        for path in paths {
            ...
            assert!(skill.contains(&path), "Meeki skill is missing `{path}`");
        }
