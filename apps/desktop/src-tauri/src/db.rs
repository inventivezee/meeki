...
-> Result<Option<meeki_db_core::CloudsyncRuntimeConfig>, String> {
...
fn cloudsync_runtime_config(
    ...
    let allow_static_auth = get("MEEKI_CLOUDSYNC_ALLOW_STATIC_AUTH")
        .and_then(nonempty)
        .map(parse_env_flag)
        .transpose()?
        .unwrap_or(false);
    ...
    let database_id = get("MEEKI_CLOUDSYNC_E2EE_DATABASE_ID").and_then(nonempty);
    let api_key = get("MEEKI_CLOUDSYNC_API_KEY").and_then(nonempty);
    let token = get("MEEKI_CLOUDSYNC_TOKEN").and_then(nonempty);
    ...
    let auth = match (api_key, token) {
        ...
        (Some(_), Some(_)) => {
            return Err(
                "configure only one of MEEKI_CLOUDSYNC_API_KEY or MEEKI_CLOUDSYNC_TOKEN"
                    .to_string(),
            );
        }
    };
    let sync_interval_ms = get("MEEKI_CLOUDSYNC_INTERVAL_MS")
        .and_then(nonempty)
        .map(|value| {
            value
                .parse::<u64>()
    ...
    Ok(Some(meeki_db_core::CloudsyncRuntimeConfig {
...
fn parse_env_flag(value: String) -> Result<bool, String> {
    match value.to_ascii_lowercase().as_str() {
        "1" | "true" => Ok(true),
        "0" | "false" => Ok(false),
        _ => Err("MEEKI_CLOUDSYNC_ALLOW_STATIC_AUTH must be true, false, 1, or 0".to_string()),
    }
}
...
mod tests {
    ...
    fn dev_uses_an_isolated_persistent_database() {
        let db_dir = desktop_db_dir("com.meeki.dev").unwrap();

        assert!(db_dir.ends_with("com.meeki.dev"));
    }
    ...
    fn cloudsync_environment_config_enables_only_core_tables() {
        let values = HashMap::from([
            ("MEEKI_CLOUDSYNC_ALLOW_STATIC_AUTH", "true".to_string()),
            (
                "MEEKI_CLOUDSYNC_E2EE_DATABASE_ID",
                "managed-database-id".to_string(),
            ),
            ("MEEKI_CLOUDSYNC_TOKEN", "token".to_string()),
            ("MEEKI_CLOUDSYNC_INTERVAL_MS", "15000".to_string()),
        ]);
    ...
    fn cloudsync_environment_rejects_multiple_credentials() {
        let values = HashMap::from([
            ...
            ("MEEKI_CLOUDSYNC_API_KEY", "api-key".to_string()),
            ("MEEKI_CLOUDSYNC_TOKEN", "token".to_string()),
        ]);
