...
describe("getSessionKeywords", () => {
  it("builds keywords from the current session snapshot", async () => {
    ...
    await expect(
      getSessionKeywords({
        sessionId: "session-1",
        dictionaryTerms: ["Meeki"],
      }),
    ).resolves.toEqual(expect.arrayContaining(["Meeki", "Launch"]));
  });
  ...
  it("prioritizes mapped participants and attached event attendees", async () => {
    ...
    expect(result.slice(0, 3)).toEqual(["Alice Kim", "Mina Park", "Meeki"]);
...
describe("dictionary term helpers", () => {
  it("parses newline and comma separated terms", () => {
    expect(
      parseDictionaryTermsText("Meeki\nFastConformer, Parakeet TDT"),
    ).toEqual(["Meeki", "FastConformer", "Parakeet TDT"]);
  });

  it("normalizes duplicate terms while preserving first spelling", () => {
    expect(normalizeKeywordList(["Meeki", " meeki ", "Parakeet"])).toEqual([
      "Meeki",
      "Parakeet",
    ]);
  });
  ...
  it("formats stored terms one per line", () => {
    expect(formatDictionaryTerms(["Meeki", "Parakeet TDT"])).toBe(
      "Meeki\nParakeet TDT",
    );
  });
});
