...
describe("ChatCTA", () => {
  ...
  it("rests as a handle and expands into an input-like field on hover", () => {
    ...
    expect(surface?.className).toContain(
      "group-hover/meeki-chat-cta:shadow-[0_16px_42px_rgba(0,0,0,0.26)]",
    );
    expect(surface?.className).toContain(
      "dark:group-hover/meeki-chat-cta:shadow-[0_18px_52px_rgba(0,0,0,0.64)]",
    );
    ...
    expect(surface?.className).toContain(
      "group-hover/meeki-chat-cta:border-border/70",
    );
    expect(surface?.className).toContain(
      "group-hover/meeki-chat-cta:bg-[#f4f4f5]",
    );
    expect(surface?.className).toContain(
      "dark:group-hover/meeki-chat-cta:bg-[#202020]",
    );
    expect(surface?.className).toContain("group-hover/meeki-chat-cta:h-10");
    expect(surface?.className).toContain(
      "group-hover/meeki-chat-cta:w-[min(640px,calc(100cqw_-_2rem))]",
    );
    expect(surface?.className).toContain(
      "group-focus-visible/meeki-chat-cta:w-[min(640px,calc(100cqw_-_2rem))]",
    );
    ...
    expect(label.className).toContain(
      "group-hover/meeki-chat-cta:text-muted-foreground",
    );
  ...
  it("hides while the floating chat is open", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Ask Meeki anything" }),
    ).toBeNull();
  });
