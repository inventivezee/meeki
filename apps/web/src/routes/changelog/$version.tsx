...
export const Route = createFileRoute("/changelog/$version")({
  ...
  head: ({ loaderData }) => {
    ...
    const url = `${MEEKI_SITE_URL}/changelog/${entry.version}`;
    const description =
      entry.summary ?? `Release notes for Meeki v${entry.version}.`;

    return {
      links: [{ rel: "canonical", href: url }],
      meta: [
        { title: `Meeki v${entry.version} Changelog` },
        {
          name: "description",
          content: description,
        },
        {
          property: "og:title",
          content: `Meeki v${entry.version} Changelog`,
        },
        {
