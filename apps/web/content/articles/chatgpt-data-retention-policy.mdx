...
[Meeki](https://meeki.org/) is an open-source AI notepad for meetings that lets you bring your own API key. Connect your OpenAI, Anthropic, Google Gemini, or Azure-hosted GPT key, and your meeting data goes through the API. The retention policies from your API agreement apply, not the consumer defaults described above.

If even API-level retention is too much, Meeki also runs fully offline with local models through Ollama or LM Studio. Nothing leaves your machine.

No bot in your call. Meeki captures audio directly from your computer's input and output. No third-party bot joins your Zoom or Google Meet, so no intermediary service handles your meeting data.

Notes stay on your device by default. Transcripts and summaries are stored in local SQLite; encrypted CloudSync and sharing are optional features, so enable them only when they fit your data-handling requirements.

More than a transcription tool. Meeki does real-time transcription while you jot down what matters, then generates summaries from your memos once the meeting ends. Built-in AI chat lets you ask follow-ups—action items, rewrites, translations. It supports customizable note templates and integrates with Apple Calendar, Contacts, and Obsidian, with Notion, Slack, HubSpot, and Salesforce on the way.
...
[Download Meeki for macOS](https://meeki.org/download/apple-silicon) and use the AI provider your security team actually trusts.
