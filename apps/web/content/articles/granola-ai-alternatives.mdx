...
- **Choose Anarlog** if you want bot-free capture, local transcription, and a local-first meeting record with Markdown export.
...
| --- | --- | --- | --- | --- |
| **Anarlog** | Desktop system audio, no meeting bot | Local SQLite with Markdown export | Local-first meeting workflows | Mac-focused product experience today |
...
## 1. Anarlog: best for local-first meeting data and AI-provider control

[Anarlog](/) is an open-source, local-first meeting notetaker. It captures microphone and system audio without joining the call as a participant, transcribes the conversation, and keeps meeting data in local SQLite on your computer.
...
With Anarlog, the durable artifact is a local meeting record you can search, back up, export, or include in a broader knowledge workflow. Transcription can run locally, and AI can use local models, your own provider keys, or Anarlog's hosted service.
...
Anarlog keeps meeting data locally in SQLite and can run transcription locally. Meetily is also designed for local or self-managed processing. Notta documents a local Privacy Mode for supported plans and workflows, but that should not be confused with making every Notta workflow local.
