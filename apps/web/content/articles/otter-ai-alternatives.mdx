...
- **Choose Meeki** if you want bot-free capture, local transcription, and meeting notes stored as files you control.
...
| --- | --- | --- | --- | --- | --- |
| **Meeki** | Device audio and local transcription | No | Local SQLite with Markdown export | No | Local-first meeting workflows |
...
## 1. Meeki: best for local-first meeting memory

[Meeki](/) is an open-source, local-first meeting notetaker for people who do not want their meeting history trapped inside another proprietary dashboard.

It captures meeting audio from your device without joining the call as a participant. Transcription can run locally, and your audio, transcripts, and notes remain available as files on your computer. You can use local models, connect your own AI provider, or use Meeki’s hosted AI depending on the workflow you want.
...
Meeki and Granola both avoid a visible meeting participant. The main distinction is what happens to the record afterward: a managed collaborative workspace versus files owned by the user.
...
Meeki and Meetily share a local-first philosophy. The practical choice comes down to platform support, desired product polish, and how much of the stack you want to operate yourself.
...
For users who want local-first storage, bot-free capture, and on-device transcription, Meeki is a strong fit in this comparison. Meetily is another local, open-source option, especially for Windows and Linux users.
...
Meeki starts from a different premise: the meeting record should remain part of your own files and knowledge system.

If you want bot-free capture, local transcription, open-source software, and notes that remain available outside a proprietary meeting database, [download Meeki for macOS](/download).
