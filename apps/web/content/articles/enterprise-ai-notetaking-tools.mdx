...
1. **Meeki**: Open-source, local-first AI notepad with configurable AI processing
...
[Meeki](/) is an open-source AI notepad for meetings built for teams that want a local-first workflow and configurable AI processing. Meeting data is stored locally in SQLite instead of a vendor-hosted workspace, with Markdown export when a portable copy is useful. You can use managed cloud AI, bring your own keys, or use local models. That design can make technical review clearer, but it does not replace a vendor, security, or compliance assessment.
...
Meeki uses two types of models: speech-to-text (STT) and large language models (LLMs).
...
Meeki captures microphone input and system audio output on the desktop without adding a meeting bot. It can work alongside major meeting platforms, subject to the platform, device, and configuration in use.
...
Every enterprise has unique compliance requirements, workflow needs, and technical constraints. Meeki's flexible architecture adapts to all three. Whether you need edge-only deployment for maximum IP protection, all-prem for complete infrastructure control, or prem-hybrid to balance compliance with powerful AI, there's a configuration that fits.
