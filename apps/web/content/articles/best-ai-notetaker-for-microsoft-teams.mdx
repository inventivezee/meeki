...
- **Choose Meeki** when you want bot-free capture, local transcription, and a local-first meeting record on your own computer.
...
| Meeki | Desktop audio capture and local transcription | No | Local SQLite with Markdown export | Local-first, bot-free meeting notes |
...
Meeki is our open-source, local-first meeting notetaker for macOS. It captures the meeting from your computer, transcribes locally, and stores notes as Markdown on disk.

Because capture happens on your device, Meeki does not need to join the Teams call as a guest participant. It can be used with the Teams desktop or web experience without adding another name to the attendee list.
...
![Meeki desktop quick-start screen with New Note, Start Recording, and Settings actions](/api/assets/blog/library/products/meeki/desktop/2026-07-22-quick-start.jpg)
...
- No meeting bot
- Local transcription
- Local SQLite meeting record with Markdown export
...
Meeki is the strongest fit when "private" means controlling the artifact itself: where the transcript lives, which model sees it, and how the notes are retained.
...
### Choose Meeki when ownership comes first

Use Meeki if you want bot-free capture, local transcription, a local meeting record, and control over the AI model used for summaries.
...
Yes. A local desktop recorder such as Meeki captures from your device, while a browser-extension workflow such as Tactiq operates inside Teams on the web. Neither requires an automated guest participant.
...
[Download Meeki for macOS](/download) if you want Microsoft Teams notes without a meeting bot, with local transcription and meeting data stored on your computer.
