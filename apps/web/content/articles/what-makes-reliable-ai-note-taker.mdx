...
When we started building Meeki, we knew what we were up against. Otter, Fireflies, Granola—the space was already crowded with well-funded players offering impressive features.
...
## How we're building Meeki

We're designing Meeki around dependency thinking.

You can configure the tool for local processing with on-device transcription and local models. Managed AI, CloudSync, sharing, and integrations are optional layers; review the services you enable rather than assuming every workflow has the same data path.

**Your choice of AI stack.** Meeki works fully offline out of the box, but you're not locked into local-only. Use the managed cloud service for the easiest setup, bring your own API keys (OpenAI, Anthropic, Mistral, and others), or run local models via Ollama or LM Studio. Switch anytime. No lock-in.

**Deployment clarity.** Meeki is open source and stores meeting data locally by default. That makes the desktop data path inspectable; it does not imply a supported self-hosted or private-cloud deployment.

**Direct audio capture.** Meeki captures system audio and microphone input directly at the OS level. No bots. No platform permissions. No dependency on Zoom or Teams policies. Works for all meeting platforms and in-person conversations.
...
**Compliance requires review.** Local processing can reduce external exposure, but it does not establish HIPAA, attorney-client privilege, GDPR residency, or SOC 2 compliance. Review consent, access, retention, contracts, and every configured provider with qualified counsel and security teams.
