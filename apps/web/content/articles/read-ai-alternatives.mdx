...
| **Anarlog**              | Open-source, bot-free, local-first meeting data with AI-provider choice | Free (BYOK); Pro $15/mo or $150/year |
...
You can take notes during a meeting in Anarlog's built-in notepad while it transcribes in the background, and the AI combines both into structured output after the call. Or you can skip the manual notes entirely and let it generate summaries from the transcript alone. 

The AI stack is yours to pick: Anarlog's managed cloud service, your own API keys from OpenAI, Anthropic, or Deepgram, or fully local models through Ollama. That last option means the entire pipeline can run offline, on your machine, with nothing leaving your network. 
...
The limitation is that it only works in the browser. If your team uses native desktop apps for Zoom or Teams, Tactiq can't capture those calls. No in-person meeting support either, which is where Anarlog and Jamie have the edge.
...
If you came to Read AI for meeting notes and the bot was the problem, Anarlog provides open-source, local-first meeting data and AI-provider choice. Jamie packages a bot-free experience with CRM integrations and EU hosting; Tactiq is a browser-first option, and Granola fits a collaborative notepad workflow. Verify each product's current pricing, security, and plan limits before choosing.
...
Anarlog is free for unlimited local transcription. [Download it for free](https://anarlog.so/download/apple-silicon) and try it on your next call. No account required, no calendar permissions, no data leaves your machine unless you want it to.
