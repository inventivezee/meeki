...
meta_description: "Six open-source AI meeting notetakers you can self-host — from full meeting workspaces to raw transcription pipelines. Covers Meeki, Meetily, Scriberr, Vibe, Whishper, and WhisperX."
author:
  - "John Jeong"
featured: false
...
| **Meeki**     | MIT         | macOS           | Parakeet V3, Parakeet Batch + 7 cloud         | Ollama / LM Studio | Free (local) · $8–$25/mo cloud |
...
![Meeki review](/api/assets/blog/articles/selfhosted-ai-notetakers/char-summary.png "char-editor-width=80")

[Meeki](https://meeki.org/) is the only tool on this list that handles the full meeting workflow in one app — capture, transcription, note-taking, summarization, and structured export. 

It ships with a built-in notepad where you write during the meeting, and the AI merges your manual notes with the transcript into structured output using your template. The canonical meeting record is local SQLite, with Markdown export when you need a portable copy.

#### Setup

**Install:** brew install meeki, or clone the [MIT-licensed repo](https://github.com/inventivezee/Meety) and build from source.

**Transcription:** Pick from 9 STT providers in Preferences. Local Parakeet models run fully on-device, and the other providers are cloud BYOK: paste your API key, audio goes to that provider, Meeki never touches it.
...
**Storage:** Local SQLite, with attachments and recordings kept as local files
...
Free and open-source (MIT). The desktop application is local-first; do not treat it as a supported self-hosted server product.
...
- If you want a full meeting workspace, Meeki is the most complete option on this list. 
...
For most users who want privacy-first meeting notes that just work, start with [Meeki](https://meeki.org/download/apple-silicon). Free for fully local use, no account required.
