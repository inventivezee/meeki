...
| **Meeki**       | Users who want a local-first meeting workflow and AI-provider choice | Free (BYOK), Pro $15/mo or $150/year |
...
[Meeki](https://meeki.org) is an open-source AI notepad for meetings, built for people who want complete control over their data and AI stack. It captures system audio from any meeting platform without joining your call and without requiring calendar permissions. 

You can take notes during the meeting in Meeki's built-in editor while it transcribes in the background, and the AI combines both into a structured summary after the call. 

The AI stack is yours to choose: Meeki's managed cloud service, your own API keys, or fully local models through Ollama or LM Studio for offline operation. The canonical meeting record stays in local SQLite; attachments and recordings remain local files, and you decide whether to enable cloud services.
...
- Storage: the canonical meeting record is local SQLite on your device, with portable exports when you need them.
- Open-source codebase. Your security team can audit exactly how data is handled.

If you use on-device transcription and local models without enabling CloudSync or sharing, meeting content does not need to be sent to an AI provider. If you use an API key or managed AI, review what that provider receives. For organizations restricting cloud meeting tools, validate the exact configuration with security and legal teams.
...
- CloudSync and sharing are optional features; the canonical meeting record remains local by default.
...
Meeki is the strongest choice if you want complete control: open-source, your choice of AI provider, and data that lives on your device in a format you own. 
...
If you want to start with local AI meeting notes that give you full ownership, [try Meeki](https://meeki.org). It is free, open-source, and works offline with local models. Your first meeting's notes will land on your device, readable by any tool you already use.
...
Yes. Tools that capture microphone input (Meeki, Meetily, Talat, Screenpipe) work for in-person conversations. Place your laptop near the speakers and let the mic pick up the room. Speaker diarization accuracy varies since it was primarily designed for virtual calls with separate audio channels.

### 3. Do local meeting notetakers work offline?

If both transcription and summarization use local models, yes. Meeki with Ollama, Meetily with Parakeet + Ollama, Talat with its built-in LLM, and Screenpipe with local Whisper all work without an internet connection. Shadow's transcription works offline but the AI summary features need a connection.
