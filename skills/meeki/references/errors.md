...
Run `meeki --json doctor`. Ask the user to open Meeki once if the database does not exist. If they keep data in a custom location, use `--db-path FILE` or `MEEKI_DB_PATH` after they provide the path.
...
Run `meeki --json meetings list` to distinguish database access from client configuration. Confirm the MCP command is `meeki` and its only required argument is `mcp`.
