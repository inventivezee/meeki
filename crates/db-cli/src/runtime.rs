...
pub async fn run(args: Args) -> Result<()> {
    ...
    let db = meeki_db_core::Db::connect_local_plain(&db_path)
        .await
        .map_err(|e| Error::operation_failed("open database", e.to_string()))?;

    meeki_db_app::prepare_schema(&db)
        .await
...
async fn run_templates(pool: &sqlx::SqlitePool, command: TemplateCommand) -> Result<()> {
    match command {
        TemplateCommand::List => {
            let rows = meeki_db_app::list_templates(pool)
                .await
        ...
        TemplateCommand::Get { id } => {
            let row = meeki_db_app::get_template(pool, &id)
                .await
        ...
        TemplateCommand::Delete { id } => {
            meeki_db_app::delete_template(pool, &id)
                .await
...
impl From<meeki_db_app::TemplateRow> for TemplateOutput {
    fn from(value: meeki_db_app::TemplateRow) -> Self {
        Self {
            id: value.id,
            title: value.title,
...
async fn upsert_template(pool: &sqlx::SqlitePool, args: UpsertTemplateArgs) -> Result<()> {
    ...
    meeki_db_app::upsert_template(
        pool,
        meeki_db_app::UpsertTemplate {
            id: &id,
            title: &args.title,
...
mod tests {
    ...
    async fn run_migrates_and_roundtrips_templates() {
        ...
        let db = meeki_db_core::Db::connect_local_plain(&db_path)
            .await
            .unwrap();
        let row = meeki_db_app::get_template(db.pool(), "template-1")
            .await
            .unwrap()
            .unwrap();
