...
pub fn render_enhance_system(input: &EnhanceSystem) -> Result<String, Error> {
    ...
    Ok(template.render(context! {
        current_date => meeki_askama_utils::current_date_value(),
        language => meeki_askama_utils::language_name(input.language.as_deref()),
    })?)
}
...
mod tests {
    ...
    use meeki_askama_utils::tpl_snapshot;
    ...
    fn test_enhance_system_formatting() {
        meeki_askama_utils::set_current_date_override(Some("2025-01-01".to_string()));
        let rendered = render_enhance_system(&EnhanceSystem {
            language: None,
            prompt_override: String::new(),
        })
        .unwrap();
        meeki_askama_utils::set_current_date_override(None);
