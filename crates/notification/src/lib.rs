...
pub use meeki_notification_interface::*;
...
fn resolve_default_icon(
    notification: &meeki_notification_interface::Notification,
) -> meeki_notification_interface::Notification {
...
fn show_inner(notification: &meeki_notification_interface::Notification) {
    #[cfg(all(feature = "legacy", target_os = "macos"))]
    meeki_notification_macos::show(notification);

    #[cfg(all(feature = "legacy", target_os = "linux"))]
    meeki_notification_linux::show(notification);
}
...
pub fn clear() {
    #[cfg(all(feature = "legacy", target_os = "macos"))]
    meeki_notification_macos::dismiss_all();

    #[cfg(all(feature = "legacy", target_os = "linux"))]
    meeki_notification_linux::dismiss_all();
}
    ...
    F: Fn(NotificationContext) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_dismiss_handler(move |key, _tag| {
            f(get_context(&key));
        });
    }
    ...
    {
        let f = f.clone();
        meeki_notification_linux::setup_notification_dismiss_handler(move |key| {
            f(get_context(&key));
        });
    }
    ...
    F: Fn(NotificationContext) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_collapsed_confirm_handler(move |key, _tag| {
            f(get_context(&key));
        });
    }
    ...
    {
        let f = f.clone();
        meeki_notification_linux::setup_notification_confirm_handler(move |key| {
            f(get_context(&key));
        });
    }
    ...
    F: Fn(NotificationContext) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_expanded_accept_handler(move |key, _tag| {
            f(get_context(&key));
        });
    }
    ...
    {
        let f = f.clone();
        meeki_notification_linux::setup_notification_accept_handler(move |key| {
            f(get_context(&key));
        });
    }
    ...
    F: Fn(NotificationContext) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_collapsed_timeout_handler(move |key, _tag| {
            f(get_context(&key));
        });
    }
    ...
    {
        let f = f.clone();
        meeki_notification_linux::setup_notification_timeout_handler(move |key| {
            f(get_context(&key));
        });
    }
    ...
    F: Fn(NotificationContext, i32) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_option_selected_handler(move |key, tag| {
    ...
    F: Fn(NotificationContext) + Send + Sync + 'static,
{
    ...
    {
        let f = f.clone();
        meeki_notification_macos::setup_footer_action_handler(move |key, _tag| {
            f(get_context(&key));
        });
    }
