...
use meeki_audio_utils::{Source, f32_to_i16, mono_frames};
use meeki_resampler::{
    ...
    W: Write,
{
    ...
    if channel_count == 2 {
        let mut encoder =
            meeki_mp3::StereoStreamEncoder::new(TARGET_SAMPLE_RATE_HZ).map_err(mp3_err)?;
        ...
        let mut encoder =
            meeki_mp3::MonoStreamEncoder::new(TARGET_SAMPLE_RATE_HZ).map_err(mp3_err)?;
        let mut output = Mp3Output::new(
            output,
...
fn mp3_err(e: meeki_mp3::Error) -> Error {
...
fn create_mono_resampler(source_rate: u32) -> Result<Async<f32>, Error> {
    ...
    .map_err(meeki_resampler::Error::from)?)
}
...
impl ResamplerState {
    ...
    fn encode_chunk(
        &mut self,
        encoder: &mut meeki_mp3::MonoStreamEncoder,
...
fn encode_stereo_chunk<W: Write>(
    encoder: &mut meeki_mp3::StereoStreamEncoder,
    left: &[f32],
    right: &[f32],
