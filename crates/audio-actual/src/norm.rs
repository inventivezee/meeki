...
#[pin_project]
pub struct NormalizedSource<S: meeki_audio_interface::AsyncSource> {
    source: S,
    gain_linear: f32,
    ebur128: EbuR128,
...
pub trait NormalizeExt<S: meeki_audio_interface::AsyncSource> {
    fn normalize(self) -> NormalizedSource<S>;
}

impl<S: meeki_audio_interface::AsyncSource> NormalizeExt<S> for S {
    fn normalize(self) -> NormalizedSource<S> {
...
impl<S: meeki_audio_interface::AsyncSource> Stream for NormalizedSource<S> {
    type Item = f32;
...
impl<S: meeki_audio_interface::AsyncSource> meeki_audio_interface::AsyncSource
