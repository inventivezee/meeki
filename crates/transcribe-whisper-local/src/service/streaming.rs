...
use meeki_audio_chunking::{SpeechChunkExt, SpeechChunkingConfig};
use meeki_audio_interface::AsyncSource;
use meeki_model_manager::{ModelManager, ModelManagerBuilder};
use meeki_transcribe_core::TARGET_SAMPLE_RATE;
use meeki_ws_utils::ConnectionManager;
...
pub struct TranscribeService {
    model_path: PathBuf,
    manager: ModelManager<meeki_whisper_local::LoadedWhisper>,
...
async fn handle_websocket(
    ...
    guard: meeki_ws_utils::ConnectionGuard,
    model: Arc<meeki_whisper_local::LoadedWhisper>,
    manager: ModelManager<meeki_whisper_local::LoadedWhisper>,
) {
    let (mut ws_sender, mut ws_receiver) = socket.split();
    let total_channels = (params.channels as usize).max(1);
    let redemption_time = redemption_time(&params);
    let languages: Vec<meeki_whisper::Language> = params
        .languages
        .iter()
                        ...
                        match process_incoming_message(&message, params.channels.max(1)) {
                            ...
                            Ok(IncomingMessage::Audio(AudioExtract::Dual { ch0, ch1 })) => {
                                if total_channels >= 2 {
                                    ...
                                    let mixed = meeki_audio_utils::mix_audio_f32(&ch0, &ch1);
...
fn build_transcription_streams(
    ...
    loaded_model: &meeki_whisper_local::LoadedWhisper,
    languages: &[meeki_whisper::Language],
...
struct TranscribeChannelStream<S> {
    channel_idx: usize,
    chunk_stream: S,
    model: meeki_whisper_local::Whisper,
    pending: VecDeque<crate::service::Segment>,
}

impl<S> TranscribeChannelStream<S> {
    fn new(channel_idx: usize, chunk_stream: S, model: meeki_whisper_local::Whisper) -> Self {
        Self {
            channel_idx,
            chunk_stream,
            model,
...
where
    S: Stream<Item = Result<meeki_audio_chunking::AudioChunk, meeki_audio_chunking::Error>> + Unpin,
{
