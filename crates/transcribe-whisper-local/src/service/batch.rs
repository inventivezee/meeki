...
use bytes::Bytes;
use meeki_model_manager::ModelManager;
...
fn transcribe_batch(
    ...
    let extension = meeki_audio_utils::content_type_to_extension(content_type);
    ...
    let source = meeki_audio_utils::source_from_path(temp_file.path())?;
...
pub(super) fn transcribe_recorded_file(
    ...
    let source = meeki_audio_utils::source_from_path(audio_path)?;
    ...
    S: Source<Item = f32>,
{
    ...
    let resampled = meeki_audio_utils::resample_audio(source, TARGET_SAMPLE_RATE)?;
...
fn transcribe_chunks(
    channel_idx: usize,
    chunks: &[meeki_audio_chunking::AudioChunk],
    channel_duration: f64,
    model: &mut meeki_whisper_local::Whisper,
