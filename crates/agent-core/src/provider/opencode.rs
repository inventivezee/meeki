...
pub fn health(options: &HealthCheckOptions) -> ProviderHealth {
    let health = meeki_opencode::health_check_with_options(&meeki_opencode::OpencodeOptions {
...
pub fn install_cli() -> Result<InstallCliResponse, String> {
    let plugin_path = meeki_opencode::plugin_path();

    if plugin_path.exists() && !meeki_opencode::is_char_plugin(&plugin_path)? {
        return Err(format!(
    ...
    meeki_opencode::write_plugin(&plugin_path)?;
...
pub fn uninstall_cli() -> Result<UninstallCliResponse, String> {
    ...
    if plugin_path.exists() && !meeki_opencode::has_char_plugin(&plugin_path)? {
        return Err(format!(
    ...
    meeki_opencode::remove_plugin(&plugin_path)?;
...
pub fn upgrade() {
    upgrade_at(&meeki_opencode::plugin_path());
}

fn upgrade_at(plugin_path: &std::path::Path) {
    if meeki_opencode::is_char_plugin(plugin_path).unwrap_or(false) {
        let _ = meeki_opencode::write_plugin(plugin_path);
    }
}

fn integration_installed() -> Result<bool, String> {
    let plugin_path = meeki_opencode::plugin_path();
    meeki_opencode::is_char_plugin(&plugin_path)
}

impl From<meeki_opencode::HealthStatus> for ProviderHealthStatus {
    fn from(value: meeki_opencode::HealthStatus) -> Self {
        match value {
            meeki_opencode::HealthStatus::Ready => Self::Ready,
            meeki_opencode::HealthStatus::Warning => Self::Warning,
            meeki_opencode::HealthStatus::Error => Self::Error,
        }
    }
}

impl From<meeki_opencode::HealthAuthStatus> for ProviderAuthStatus {
    fn from(value: meeki_opencode::HealthAuthStatus) -> Self {
        match value {
            meeki_opencode::HealthAuthStatus::Authenticated => Self::Authenticated,
            meeki_opencode::HealthAuthStatus::Unauthenticated => Self::Unauthenticated,
            meeki_opencode::HealthAuthStatus::Unknown => Self::Unknown,
        }
    }
}
...
mod tests {
    ...
    fn upgrade_refreshes_existing_plugin() {
        ...
        assert!(meeki_opencode::has_char_plugin(&path).unwrap());
    }
}
