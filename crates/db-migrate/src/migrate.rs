            ...
            match scope {
                ...
                MigrationScope::CloudsyncAlter {
                    ...
                    let cloudsync_table_enabled = self.db.cloudsync_enabled()
                        && meeki_db_core::cloudsync_is_enabled_on(&mut *self.conn, cs_table)
                            .await
                    ...
                    meeki_db_core::cloudsync_begin_alter_on(&mut *self.conn, cs_table)
                        .await
                        .map_err(cloudsync_error)?;

                    execute_migration(&mut self.conn, migration).await?;

                    meeki_db_core::cloudsync_commit_alter_on(&mut *self.conn, cs_table)
                        .await
                        .map_err(cloudsync_error)?;
