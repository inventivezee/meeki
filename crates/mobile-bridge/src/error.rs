...
pub(crate) fn execute_error(error: meeki_db_execute::Error) -> BridgeError {
    BridgeError::QueryFailed {
        reason: error.to_string(),
    }
}

pub(crate) fn reactive_error(error: meeki_db_reactive::Error) -> BridgeError {
    BridgeError::QueryFailed {
        reason: error.to_string(),
    }
}
...
pub(crate) fn cloudsync_runtime_error(error: meeki_db_core::CloudsyncRuntimeError) -> BridgeError {
    BridgeError::CloudsyncFailed {
        reason: error.to_string(),
    }
}
...
pub(crate) enum OpenAppDbError {
    #[error(transparent)]
    Open(#[from] meeki_db_core::DbOpenError),
    #[error(transparent)]
