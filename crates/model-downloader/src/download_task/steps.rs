...
) -> Result<(), meeki_file::Error> {
    meeki_file::download_file_parallel_cancellable(
        &params.url,
        &params.destination,
        progress_callback,
...
pub(super) async fn verify_checksum<M: DownloadableModel>(
    ...
    let checksum_result = tokio::task::spawn_blocking(move || {
        meeki_file::calculate_file_checksum(destination_for_checksum)
    })
    .await;
