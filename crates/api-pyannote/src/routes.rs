...
use meeki_pyannote_cloud::ClientInfo;
...
#[derive(Clone)]
struct AppState {
    client: meeki_pyannote_cloud::Client,
}
...
) -> Result<meeki_pyannote_cloud::types::DiarizeRequest> {
...
) -> Result<meeki_pyannote_cloud::types::IdentifyRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
...
) -> Result<meeki_pyannote_cloud::types::VoiceprintRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
