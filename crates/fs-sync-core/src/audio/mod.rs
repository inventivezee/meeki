...
pub fn source_metadata(source_path: &Path) -> std::io::Result<AudioSourceMetadata> {
    ...
    let duration_ms = meeki_audio_utils::source_from_path(source_path)
        .ok()
...
pub fn import_to_session(
    ...
    let result = meeki_audio_norm::normalize_file(
        source_path,
        &tmp_path,
        &target_path,
        None,
...
pub fn import_audio(
    ...
    tmp_path: &Path,
    target_path: &Path,
) -> Result<PathBuf, meeki_audio_norm::Error> {
    meeki_audio_norm::normalize_file(source_path, tmp_path, target_path, None, None::<fn(f64)>)
}
...
mod tests {
    ...
    test_import_audio! {
        test_import_wav: meeki_data::english_1::AUDIO_PATH,
        test_import_mp3: meeki_data::english_1::AUDIO_MP3_PATH,
        test_import_mp4: meeki_data::english_1::AUDIO_MP4_PATH,
        test_import_m4a: meeki_data::english_1::AUDIO_M4A_PATH,
        test_import_ogg: meeki_data::english_1::AUDIO_OGG_PATH,
        test_import_flac: meeki_data::english_1::AUDIO_FLAC_PATH,
    ...
    fn test_import_stereo_mp3() {
        let source_path = std::path::Path::new(meeki_data::english_10::AUDIO_MP3_PATH);
        let temp = TempDir::new().unwrap();
        let tmp_path = temp.path().join("tmp.mp3");
        let target_path = temp.path().join("target.mp3");
        ...
        let decoder = meeki_audio_utils::source_from_path(&target_path).unwrap();
