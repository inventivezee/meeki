use meeki_api_env::{NangoEnv, SupabaseEnv};
use meeki_nango::NangoClient;
...
pub(crate) fn build_nango_client(config: &NangoConfig) -> Result<NangoClient, meeki_nango::Error> {
    let mut builder = meeki_nango::NangoClient::builder().api_key(&config.nango.nango_api_key);
