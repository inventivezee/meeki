...
impl LocalSoniqoLiveClient {
    pub fn new(model: meeki_transcribe_soniqo::SoniqoModel) -> Self {
        Self { model }
    }
    ...
    pub async fn from_realtime_audio_single(
        ...
        source: meeki_transcribe_soniqo::TranscriptSource,
...
async fn run_single(
    session: meeki_transcribe_soniqo::LiveTranscriptionSession,
    model: meeki_transcribe_soniqo::SoniqoModel,
    source: meeki_transcribe_soniqo::TranscriptSource,
...
async fn finalize_single(
    ...
    cursor: &mut f64,
    response_tx: &tokio::sync::mpsc::Sender<Result<StreamResponse, LocalSoniqoLiveError>>,
) -> Result<meeki_transcribe_soniqo::LiveTranscriptionSession, LocalSoniqoLiveError> {
...
async fn stop_after_finalize(
    result: Result<meeki_transcribe_soniqo::LiveTranscriptionSession, LocalSoniqoLiveError>,
) {
    match result {
...
async fn start_session(
    ...
    tokio::task::spawn_blocking(move || {
        meeki_transcribe_soniqo::LiveTranscriptionSession::start(model)
    })
    .await
...
async fn stop_session(session: meeki_transcribe_soniqo::LiveTranscriptionSession) {
