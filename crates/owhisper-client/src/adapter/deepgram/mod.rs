...
impl DeepgramModel {
    ...
    pub fn supports_language(&self, lang: &meeki_language::Language) -> bool {
        lang.matches_any_code(self.supported_languages())
    }

    pub fn supports_multi(&self, languages: &[meeki_language::Language]) -> bool {
...
impl DeepgramAdapter {
    pub fn find_model(languages: &[meeki_language::Language]) -> Option<DeepgramModel> {
        if languages.len() >= 2 {
    ...
    pub fn supports_batch_language_detection(languages: &[meeki_language::Language]) -> bool {
        !languages.is_empty() && languages.iter().all(language::supports_language_detection)
    }

    fn can_use_multi(languages: &[meeki_language::Language]) -> bool {
    ...
    fn single_language_support(language: &meeki_language::Language) -> LanguageSupport {
    ...
    pub fn recommended_model_live(languages: &[meeki_language::Language]) -> Option<&'static str> {
...
mod tests {
    ...
    fn test_model_supports_language() {
        let en: meeki_language::Language = ISO639::En.into();
        let zh: meeki_language::Language = ISO639::Zh.into();
    ...
    fn test_en_ca_with_nova3_general_not_supported() {
        let en_ca: meeki_language::Language = "en-CA".parse().unwrap();
    ...
    fn test_en_us_with_nova3_general_supported() {
        let en_us: meeki_language::Language = "en-US".parse().unwrap();
